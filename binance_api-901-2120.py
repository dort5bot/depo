...
-------------------------------------------------------------
Yenilikler:
1. Yeni Mimaride user_id Kaldırıldı
Artık direkt api_key ve secret_key parametreleri alıyor
Daha temiz ve anlaşılır API

2. Circuit Breaker Pattern
CircuitBreaker class'ı ile sistemik hata yönetimi
Otomatik devre kesme ve recovery mekanizması

3. Gelişmiş Hata Yönetimi
Tüm API çağrıları try-except bloklarıyla sarıldı
Detaylı logging ve error tracking

4. Priority Tabanlı İşleme
RequestPriority enum'u ile öncelik sistemi
Semaphore'larla concurrency kontrolü

5. Akıllı Retry Mekanizması
Exponential backoff ile retry
Rate limit durumunda otomatik delay

6. Gelişmiş Caching
Endpoint bazlı TTL yönetimi
Periyodik cache cleanup

7. WebSocket Otomatik Recovery
Bağlantı kopmalarında otomatik reconnect
Ping-pong mekanizması ile connection health kontrolü

8. Detaylı Metrikler
get_detailed_metrics() ile kapsamlı monitoring
HTTP, WebSocket ve circuit breaker metrikleri

9. Graceful Shutdown
Tüm bağlantıların düzgünce kapatılması
Kaynak sızıntısı önleme

10. Config Yönetimi
Mevcut config sistemine entegrasyon
Tüm parametreler config'ten yönetilebilir

profesyonel seviyede bir trading botu için gerekli tüm özellikleri içermektedir,
gerçek zamanlı trading ortamlarında güvenle kullanılabilir.
-------------------------------------------------------------

# 🟡🟡🟡
# utils/binance_api.py
'''
Binance HTTP & WebSocket client (async) - Gelişmiş Profesyonel Sürüm
...
YENİ MİMARİ: user_id parametresi KALDIRILDI, direkt api_key/secret_key alıyor
'''
